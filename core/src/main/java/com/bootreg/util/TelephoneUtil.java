...
public class TelephoneUtil {
    ...
    /**
     * 比较版本号,这个函数只是单纯比较版本号，所以key最好用各个产家特有的key,如果是使用公用的key
     * 请先自行判断一下手机型号或产家后，再使用这个函数进行版本号大小比较
     *
     * @param version 需要进行比较的版本号
     * @param phoneV     手机的版本号
     * @return 如果version2 比 version大，就返回大于正数，相等返回0，小返回负数,无法比较返回CANNOT_COMPARE
     */
    ...
    public static int compareToVersion(String version, String version2) {
        if (TextUtils.isEmpty(version)) return CANNOT_COMPARE;
        if (version2 == null) return CANNOT_COMPARE;
        String[] version_base = version.split("\\.");
        String[] version_compare = version2.split("\\.");
        int count = version_base.length <= version_compare.length ? version_base.length : version_compare.length;

        try {
            for (int i = 0; i < count; i++) {
                int version_base_int = Integer.parseInt(version_base[i]);
                int version_compare_int = Integer.parseInt(version_compare[i]);
                if (version_compare_int > version_base_int) {
                    return 1;
                } else if (version_compare_int == version_base_int) {
                    continue;
                } else {
                    return -1;
                }
        ...
        if (version_compare.length > version_base.length) {
            return 1;
        } else if (version_compare.length == version_base.length) {
            return 0;
        } else {
            return -1;
        }
